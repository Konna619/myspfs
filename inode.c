...
static int spfs_setattr(struct user_namespace * ns, struct dentry *dentry, struct iattr *attr)
{
	...
	if (spfs_should_bypass(inode))
		return spfs_setattr_bp(ns, dentry, attr);
	...
	setattr_copy(ns, inode, attr); /* need dirty concept? */
...
static int spfs_permission(struct user_namespace * ns, struct inode *inode, int mask)
{
	if (spfs_should_bypass(inode))
		return spfs_permission_bp(ns, inode, mask);
	return generic_permission(ns, inode, mask);
}

int spfs_getattr(struct user_namespace * ns, const struct path *path, struct kstat *stat, u32 request_mask,
		unsigned int flags)
{
	struct inode *inode = d_inode(path->dentry);

	if (spfs_should_bypass(inode))
		return spfs_getattr_bp(ns, path, stat, request_mask, flags);
...
static int spfs_iomap_begin(struct inode *inode, loff_t pos,
		loff_t length, unsigned flags, struct iomap *iomap, struct iomap *iomap2)
{
