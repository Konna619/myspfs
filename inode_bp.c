...
int spfs_create_bp(struct user_namespace * ns, struct inode *dir, struct dentry *dentry,
		umode_t mode, bool excl)
{
	...
	rc = vfs_create(ns, d_inode(lower_dir_dentry), lower_dentry, mode, true);
	if (rc)
		goto out;
	...
	if (IS_ERR(inode)) {
		rc = PTR_ERR(inode);
		vfs_unlink(ns, d_inode(lower_dir_dentry), lower_dentry, NULL);
		goto out;
	}
...
int spfs_unlink_bp(struct inode *dir, struct dentry *dentry)
{
	...
	rc = vfs_unlink(lower_dir_dentry->d_sb->s_user_ns, d_inode(lower_dir_dentry), lower_dentry, NULL);
	if (rc)
		goto out;
...
int spfs_mkdir_bp(struct user_namespace * ns, struct inode *dir, struct dentry *dentry, umode_t mode)
{
	int rc;
	struct dentry *lower_dentry;
	struct dentry *lower_dir_dentry;
	...
	rc = vfs_mkdir(ns, d_inode(lower_dir_dentry), lower_dentry, mode);
	if (rc)
		goto out;
...
int spfs_rmdir_bp(struct inode *dir, struct dentry *dentry)
{
	...
	rc = vfs_rmdir(lower_dir_dentry->d_sb->s_user_ns, lower_dir_inode, lower_dentry);
	if (rc)
		goto out;
...
int spfs_setattr_bp(struct user_namespace * ns, struct dentry *dentry, struct iattr *ia)
{
	...
	rc = setattr_prepare(ns, dentry, ia);
	if (rc)
		goto out;
	...
	rc = notify_change(ns, lower_dentry, &lower_ia, NULL);
...
int spfs_getattr_bp(struct user_namespace * ns, const struct path *path, struct kstat *stat,
...
int spfs_permission_bp(struct user_namespace * ns, struct inode *inode, int mask)
{
	return inode_permission(ns, spfs_inode_to_lower(inode), mask);
}
...
static int spfs_xattr_set_bp(const struct xattr_handler *handler, struct user_namespace * ns,
		struct dentry *dentry, struct inode *inode,
		...
		int flags)
{
	...
	if (value)
		ret = vfs_setxattr(ns , lower_dentry, name, value, size, flags);
	else {
		BUG_ON(flags != XATTR_REPLACE);

		inode_lock(lower_inode);
		ret = __vfs_removexattr(ns , lower_dentry, name);
