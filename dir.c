		...
		unsigned long arg)
{
	...
	switch (cmd) {
		case SPFS_IOC_SET_USE_PM:
			ret = vfs_setxattr(lower_file->f_inode->i_sb->s_user_ns , lower_file->f_path.dentry,
		...
		case SPFS_IOC_SET_USE_DISK:
			ret = vfs_removexattr(lower_file->f_inode->i_sb->s_user_ns , lower_file->f_path.dentry,
...
static int spfs_dir_getattr(struct user_namespace * ns, const struct path *path, struct kstat *stat,
		u32 request_mask, unsigned int flags)
{
	...
	rc = spfs_getattr_bp(ns, path, stat, request_mask, flags);
	if (rc)
		return rc;
...
static int __spfs_create(struct user_namespace * ns, struct inode *dir, struct dentry *dentry, umode_t mode,
		bool excl)
{
...
static int spfs_create(struct user_namespace * ns, struct inode *dir, struct dentry *dentry, umode_t mode,
		bool excl)
{
	if (!spfs_create_interest(dentry, 0))
		return spfs_create_bp(ns, dir, dentry, mode, excl);

	return __spfs_create(ns, dir, dentry, mode, excl);
}
		...
		struct file *file, unsigned open_flags, umode_t create_mode)
{
	...
	int ret;
	int (*create)(struct user_namespace * ns, struct inode *, struct dentry *, umode_t, bool);
	...
	ret = create(dentry->d_sb->s_user_ns, dir, dentry, create_mode, excl);
	if (ret)
		goto out_dput;
...
static int spfs_rename2(struct user_namespace * ns, struct inode *old_dir, struct dentry *old_dentry,
		struct inode *new_dir, struct dentry *new_dentry,
		unsigned int flags)
{
	struct spfs_sb_info *sbi = SB_INFO(old_dir->i_sb);
