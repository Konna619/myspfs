		...
		const struct iomap_ops *ops)
{
	...
	done = iomap_file_buffered_write(iocb, iter, ops);
	return done;
}
...
struct dax_device *spfs_get_dax_device(struct super_block *sb)
{
	...
	// ret = dax_supported(sb->s_bdev, PAGE_SIZE);
	// if (!ret) {
	...
	// bdev = blkdev_get_by_path(disk_name, sb->s_mode, sb->s_type);
	// if(!bdev){
	// 	spfs_msg(sb, "Can not find block device : %s", disk_name);
	// 	return ERR_PTR(-EINVAL);
	// }

	dax_device = fs_dax_get_by_bdev(sb->s_bdev);
	if (!dax_device) {
